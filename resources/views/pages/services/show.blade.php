...
@php
    $title = $service->meta_title ?: $service->name . ' | Uprise Travel Services';
...
@section('content')
    ...
    <div class="relative bg-ink-deep overflow-hidden" style="min-height: 46vh;">
        ...
        @if ($service->hasMedia('hero'))
            <img src="{{ $service->getFirstMediaUrl('hero', 'hero') }}"
                 alt="{{ $service->name }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    @foreach ($related as $rel)
                        <a href="{{ route('services.show', $rel) }}"
                            ...
                            <span class="font-display font-bold text-xl text-charcoal-soft group-hover:text-accent transition-colors shrink-0 mt-0.5">
                                {{ str_pad($loop->iteration, 2, '0', STR_PAD_LEFT) }}
                            </span>
                            <div>
                                <h3 class="font-display font-semibold text-white text-sm mb-1 group-hover:text-accent transition-colors">
                                    {{ $rel->name }}
                                </h3>

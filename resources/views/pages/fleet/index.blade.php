...
@php
    ...
    $metaDescription = 'Browse our fleet of vehicles available for hire with a professional driver across Ghana and West Africa. Sedans, SUVs, minivans, and safari 4x4s.';
@endphp
        ...
        <div class="container-page">
            ...
            @if ($vehicles->isEmpty())
                ...
                <div class="text-center py-24">
                    <svg class="w-14 h-14 text-mist mx-auto mb-5" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach ($vehicles as $vehicle)
                        <article class="group bg-white rounded-md overflow-hidden shadow-card hover:shadow-card-hover
                            ...
                            {{-- Image --}}
                            <a href="{{ route('fleet.show', $vehicle) }}" class="block overflow-hidden aspect-video bg-charcoal shrink-0">
                                @if ($vehicle->hasMedia('hero'))
                                    <img src="{{ $vehicle->getFirstMediaUrl('hero', 'card') }}"
                                         alt="{{ $vehicle->name }}"
                                         class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                                         loading="lazy">
                                @else
                                    <div class="w-full h-full flex items-center justify-center bg-linear-to-br from-charcoal to-ink">
                                        <svg class="w-16 h-16 text-charcoal-soft/50" fill="none" stroke="currentColor" stroke-width="1" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" />
                                        </svg>
                                    </div>
                            ...
                            <div class="p-5 flex flex-col flex-1">
                                ...
                                <div class="flex items-center gap-3">
                                    @php
                                        $enquireUrl = 'https://wa.me/' . config('uprise.whatsapp.number') .
                                            '?text=' . urlencode('Hi Uprise Travel, I\'d like to enquire about the ' . $vehicle->name . '.');
                                    @endphp
    ...
    <section class="bg-accent py-10">
        <div class="container-page flex flex-col sm:flex-row items-center justify-between gap-6 text-center sm:text-left">
            <div>
                ...
                <p class="text-white/80 text-sm">We do not offer self-drive options — every booking comes with a vetted driver.</p>
            </div>

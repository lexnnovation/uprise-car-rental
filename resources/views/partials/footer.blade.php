@php
    $whatsappUrl = 'https://wa.me/' . config('uprise.whatsapp.number') . '?text=' . urlencode(config('uprise.whatsapp.default_message'));
...
<footer class="bg-ink border-t border-charcoal-soft">
        ...
        <div class="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
                ...
                <ul class="space-y-3">
                    ...
                    <li>
                        <a href="{{ $link['href'] }}" class="text-stone text-sm hover:text-white transition-colors duration-150">
                            {{ $link['label'] }}
                        </a>
                    </li>
                    @endforeach
            ...
            <div>
                ...
                <ul class="space-y-3 text-sm text-stone">
                    <li>
                        <a href="tel:{{ $contact['phone_e164'] }}" class="hover:text-white transition-colors duration-150">
                            {{ $contact['phone'] }}
                        </a>
                    </li>
                    <li>
                        <a href="mailto:{{ $contact['email'] }}" class="hover:text-white transition-colors duration-150">
                            {{ $contact['email'] }}
                        </a>
                    </li>
                ...
                <div class="flex gap-4 mt-6">
                    @if($social['instagram'])
                    <a href="{{ $social['instagram'] }}" target="_blank" rel="noopener" class="text-stone hover:text-white transition-colors text-xs font-semibold tracking-wide uppercase">IG</a>
                    @endif
                    @if($social['facebook'])
                    <a href="{{ $social['facebook'] }}" target="_blank" rel="noopener" class="text-stone hover:text-white transition-colors text-xs font-semibold tracking-wide uppercase">FB</a>
                    @endif
                    @if($social['linkedin'])
                    <a href="{{ $social['linkedin'] }}" target="_blank" rel="noopener" class="text-stone hover:text-white transition-colors text-xs font-semibold tracking-wide uppercase">LI</a>
                    @endif
                    @if($social['tiktok'])
                    <a href="{{ $social['tiktok'] }}" target="_blank" rel="noopener" class="text-stone hover:text-white transition-colors text-xs font-semibold tracking-wide uppercase">TT</a>
                    @endif
                </div>
                @endif
            </div>
    ...
    <div class="border-t border-charcoal-soft">
        <div class="container-page py-5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-stone">

...
<head>
    ...
    @production
        @if (config('uprise.analytics.ga4_id'))
            <script async src="https://www.googletagmanager.com/gtag/js?id={{ config('uprise.analytics.ga4_id') }}"></script>
            <script>
                window.dataLayer = window.dataLayer || [];
                function gtag() { dataLayer.push(arguments); }
                gtag('js', new Date());
                gtag('config', '{{ config('uprise.analytics.ga4_id') }}');
            </script>
        @endif
        @if (config('uprise.analytics.clarity_id'))
